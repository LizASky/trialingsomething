...
if st.session_state.result:
    ...
    # Summary displayed right after test
    ...
    with tab1:
        ...
        st.dataframe(pd.DataFrame(telem_info).set_index("Field"))

    with tab2:
        st.header("Agentic AI Reasoning")
        st.markdown(res["diagnostic_trace_text"])
        st.subheader("Connection Metrics Trend")
        df_hist = pd.DataFrame(res["history"]).set_index("timestamp")
    ...
    with tab4:
        ...
        st.text(res["detailed_advisor_message"])

    with tab5:
        st.header("Customer View")
        # Simplified for customers: just actionable advice
        customer_advice = {
            "Weak WiFi signal": "Try moving your device closer to your router or WiFi booster.",
            "WiFi interference detected": "Try changing WiFi channels or reducing interference sources.",
            "Unstable connection detected": "Monitor your connection and retest later.",
            "Engineer visit required": "An engineer visit will be scheduled to fix your issue.",
            "Replacement recommended": "We recommend replacing the faulty equipment.",
            "Known outage detected": "There is a known service outage; please wait for updates.",
            "Test incomplete": "Please try the test again or contact support if problem persists.",
            "No issue found": "Your connection looks healthy. No action needed at this time."
        }
        st.success(customer_advice.get(action["Outcome"], action["Customer_message"]))
        st.markdown(f"### Next steps: {action['Action']}")
