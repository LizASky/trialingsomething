...
def build_marker_results(t):
    ...
    results.append({"Marker": "Retransmission rate",
...
def build_hypotheses(t, marker_results):
    ...
    hypotheses = []
    if t["known_outage"]:
        hypotheses.append({"Hypothesis": "Known service outage",
                           "Evidence": ["Known outage active"],
                           "Confidence": "High"})
        return hypotheses
    if lookup["Telemetry freshness"]["RAG"] == "Grey":
        hypotheses.append({"Hypothesis": "Telemetry missing or stale",
                           "Evidence": ["Telemetry too old/unavailable"],
                           "Confidence": "High"})
    if lookup["Equipment health"]["RAG"] == "Red":
        hypotheses.append({"Hypothesis": "Equipment replacement needed",
                           "Evidence": ["Suspected equipment fault", "Issue repeated"],
                           "Confidence": "High"})
    if lookup["Line health"]["RAG"] == "Red":
        hypotheses.append({"Hypothesis": "Engineer visit needed",
                           "Evidence": ["Line health failed", "Symptoms severe"],
                           "Confidence": "High"})
    if lookup["WiFi signal strength"]["RAG"] in ["Amber", "Red"]:
        hypotheses.append({"Hypothesis": "Poor WiFi signal or placement",
                           "Evidence": [f"WiFi signal flagged {lookup['WiFi signal strength']['RAG']}"],
                           "Confidence": "Medium"})
    if lookup["Packet loss"]["RAG"] == "Red" and lookup["Retransmission rate"]["RAG"] in ["Amber", "Red"]:
        hypotheses.append({"Hypothesis": "WiFi interference or congestion",
                           "Evidence": ["Elevated packet loss and retransmissions"],
                           "Confidence": "Medium"})
    if lookup["Rapid reconnects"]["RAG"] in ["Amber", "Red"]:
        hypotheses.append({"Hypothesis": "Intermittent connection instability",
                           "Evidence": ["Frequent rapid reconnects"],
                           "Confidence": "Medium"})
    if not hypotheses:
        hypotheses.append({"Hypothesis": "Connection healthy",
                           "Evidence": ["No Red markers detected", "Telemetry fresh", "Line health OK"],
                           "Confidence": "High"})
    return hypotheses

def choose_best_hypothesis(hypotheses):
    rank = {"High": 3, "Medium": 2, "Low": 1}
    return sorted(hypotheses, key=lambda h: rank.get(h["Confidence"], 0), reverse=True)[0]

def decide_action(t, chosen, overall_rag):
    h = chosen["Hypothesis"]
    if t["known_outage"]:
        return {"Outcome": "Known outage detected",
                "Action": "No action needed; issue is being resolved.",
                "Customer_message": "A known service outage is affecting your connection.",
                "Advisor_message": "Known outage active; no escalation needed.",
                "Agentic_level": "Suppress troubleshooting",
                "Risk": "Low"}
    if "Telemetry missing or stale" in h:
        return {"Outcome": "Test incomplete",
                "Action": "Retry the test; escalate if issue persists.",
                "Customer_message": "The test could not complete due to telemetry age.",
                "Advisor_message": "Telemetry stale or missing; diagnosis unavailable.",
                "Agentic_level": "Retry",
                "Risk": "Low"}
    if "Equipment replacement needed" in h:
        return {"Outcome": "Replacement recommended",
                "Action": "Offer replacement equipment.",
                "Customer_message": "Faulty equipment suspected; replacement recommended.",
                "Advisor_message": "Equipment fault suspected; replacement advised.",
                "Agentic_level": "Recommend replacement",
                "Risk": "Medium"}
    if "Engineer visit needed" in h:
        return {"Outcome": "Engineer visit required",
                "Action": "Schedule engineer visit.",
                "Customer_message": "An issue requires engineer intervention.",
                "Advisor_message": "Line failure detected; escalate to engineering.",
                "Agentic_level": "Escalate",
                "Risk": "High"}
    if "Poor WiFi signal or placement" in h:
        return {"Outcome": "Weak WiFi signal",
                "Action": "Move device closer to router or add WiFi booster.",
                "Customer_message": "Low WiFi signal detected; improve device placement.",
                "Advisor_message": "WiFi issues noted; recommend in-home fixes.",
                "Agentic_level": "Recommend fix",
                "Risk": "Low"}
    if "WiFi interference or congestion" in h:
        return {"Outcome": "WiFi interference detected",
                "Action": "Optimize WiFi setup and retest.",
                "Customer_message": "Potential WiFi interference or congestion.",
                "Advisor_message": "Packet loss and retransmissions indicate congestion.",
                "Agentic_level": "Recommend optimization",
                "Risk": "Low"}
    if "Intermittent connection instability" in h:
        return {"Outcome": "Unstable connection detected",
                "Action": "Monitor and retest; escalate if persists.",
                "Customer_message": "Connection instability detected; monitor connection.",
                "Advisor_message": "Instability detected; escalated if recurring.",
                "Agentic_level": "Monitor",
                "Risk": "Medium"}
    return {"Outcome": "No issue found",
...
if st.session_state.result:
    ...
    with tab1:
        ...
        telemetry_info = [
            ...
            {"Field": "Packet Loss", "Value": safe_value(telemetry["packet_loss"], "%"), "Description": "Percentage of lost data packets"},
            {"Field": "Retransmission Rate", "Value": safe_value(telemetry["retransmission_rate"], "%"), "Description": "Frequency of retransmissions"},
            {"Field": "Rapid Reconnects", "Value": safe_value(telemetry["rapid_reconnects"]), "Description": "Number of rapid reconnect events"},
            {"Field": "Telemetry Age", "Value": f"{telemetry['telemetry_age_minutes']} minutes", "Description": "Data freshness"},
            {"Field": "Line Health", "Value": telemetry["line_health"], "Description": "Broadband line health"},
            {"Field": "Known Outage", "Value": "Yes" if telemetry["known_outage"] else "No", "Description": "Known service outages"},
            {"Field": "Equipment Health", "Value": telemetry["equipment_health"], "Description": "Equipment condition"},
            {"Field": "Customer Symptom", "Value": telemetry["customer_symptom"], "Description": "Reported complaint"},
            {"Field": "Customer Impact Score", "Value": f"{telemetry['customer_impact_score']}/10", "Description": "Impact severity"},
        ]
